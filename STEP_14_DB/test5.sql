...
select department_id, department_name from departments where department_id = 90;-- Executive
...
from employees
where job_id='ST_MAN'
and salary <  any (select distinct min(salary) from employees where job_id = 'SA_REP')
...
from employees
where job_id in('FI,ACCOUNT','SA_REP')--FI_ACCOUNT' 또는 'SA_REP
...
--- union ,in,not in
select employee_id "사원번호", first_name ||' '||last_name "이름", '관리자' as "구분"
from employees
where employee_id in (select manager_id from employees)
union
select employee_id "사원번호", first_name ||' '||last_name "이름", '직원' as "구분"
from employees
...
--상관쿼리 
--메인쿼리 한행을 읽고 해당값을 서브 쿼리에서 참조하여 서브쿼리 결과에 존재하면 true를 반환
--  exists, not exists : 존재 여부에 따라 true,false 를 반환

--select null : 조회할 테이블에서 값이 입력되지 않은 데이터를 검색할때 사용
select employee_id "사원번호", first_name ||' '||last_name "이름", '관리자' as "구분"	
from employees e
